...
def train(opt):
    ...
    model = MainModel(opt).cuda()
    print("----------------------------")
    criterion = SelectLoss(opt.loss)
    
    if opt.finetune:
        params = [{"params": [param for name, param in model.named_parameters()
                              if "ttm" in name], "lr": opt.learning_rate / 10},  
                  {"params": [param for name, param in model.named_parameters()
                              if "ttm" not in name], "lr": opt.learning_rate}]  
    ...
    for epoch in range(0,opt.epochs):
        ...
        for i, (batch_img1, batch_img2, batch_label1, batch_label2, _) in enumerate(train_tbar):
            ...
            _, outs = model(batch_img1, batch_img2)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser('main model train')
    parser.add_argument("--inplanes", type=str, default=128) 
    parser.add_argument("--loss", type=str, default="bce+dice")
    parser.add_argument("--pretrain", type=str, default="") 
    #runs_LEVIR_CD/train/2/best_ckp/epoch_431_0.92888.pt
    parser.add_argument("--cuda", type=str, default="0")
    parser.add_argument("--dataset-dir", type=str, default="../autodl-tmp/LEVIR-CD/")

        ...
        for i, (batch_img1, batch_img2, batch_label1, batch_label2, name) in enumerate(eval_tbar):
            ...
            for j, (cd_pred, label) in enumerate(zip(cd_preds, labels)):
                ...
                if(count == 0):
                    ...
                    directory = "pic_BOT_CL"
                    if not os.path.exists(directory):
                        os.makedirs(directory)
                    cv2.imwrite(f"{directory}/{name}", img_colour.astype(np.uint8))
...
if __name__ == "__main__":
    ...
    # 配置测试参数
    parser.add_argument("--ckp-paths", type=str,default=["./runs_decoder_only/train/1/best_ckp/epoch_451_0.92092.pt",])
